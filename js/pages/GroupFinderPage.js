...
export const GroupFinderPage = {
            ...
            <div class="page-container finder-page-layout fade-in">
                ...
                <aside class="finder-sidebar">
                    <div class="sidebar-header">
                        <h2 data-i18n="home.title">${i18n.t('home.title')}</h2>
                        <p class="text-dim-mini" data-i18n="home.subtitle">${i18n.t('home.subtitle')}</p>
                    </div>
                    ...
                    <div class="filter-group">
                        ...
                        <select id="filter-server" class="input-tech">
                            ...
                            ${CONFIG.SERVERS.map(srv => `<option value="${srv}">${srv}</option>`).join('')}
                        </select>
                    </div>

                    <div class="filter-group">
                        <label class="label-tech" data-i18n="filters.search">${i18n.t('filters.search')}</label>
                    ...
                    <div class="filter-group">
                        ...
                        <select id="filter-level" class="input-tech">
                            <option value="all" data-i18n="ui.any">${i18n.t('ui.any')}</option>
                    ...
                    <div class="sidebar-footer" style="margin-top: auto; padding-top: 20px;">
                        <button class="btn btn-accent btn-block" id="btn-create-group">
                            <span data-i18n="home.btn_create_group">${i18n.t('home.btn_create_group')}</span>
                        </button>
                    </div>
                </aside>
    ...
    afterRender: async () => {
        ...
        const preSelectedDungeonId = getQueryParam('dungeon');

        // Reset state strictly on navigation
        GroupFinderPage.selectedDungeon = null;
        if (preSelectedDungeonId) {
            // We'll load it later at the end of afterRender or in fetchGroups
        }
        ...
        const applyFilters = () => {
            ...
            if (listContainer) {
                listContainer.innerHTML = filtered.length > 0 
                    ...
                    : `<div class="empty-state"><p data-i18n="ui.no_results">${i18n.t('ui.no_results')}</p></div>`;
                ...
                if (badge) {
                    ...
                    if (dungeon) {
                        ...
                        badge.innerHTML = `
                            <img src="${CONFIG.BASE_PATH}/assets/mazmos/${dungeon.id}.png" class="dung-badge-icon" onerror="this.src='${CONFIG.BASE_PATH}/assets/classes/icons/8.png'">
                        ...
                        document.getElementById('clear-dung-badge').onclick = () => {
                            ...
                            if (window.location.search.includes('dungeon=')) {
        ...
        const urlParams = new URLSearchParams(window.location.search);
        const preSelectedDungeonIdFromURL = urlParams.get('dungeon');

        if (preSelectedDungeonIdFromURL) {
            API.getDungeons().then(dungeons => {
                // Verify we are still on a path that expects THIS dungeon
                if (window.location.pathname !== currentPath) return;

                const dungeon = dungeons.find(d => d.id == preSelectedDungeonIdFromURL);
                if (dungeon) {
                    GroupFinderPage.selectedDungeon = dungeon;
                    applyFilters();
                }
            });
        } else if (preSelectedDungeonId) { // Fallback for hash-based or initial preSelectedDungeonId
             API.getDungeons().then(dungeons => {
                 // Verify we are still on a path that expects THIS dungeon
                 if (window.location.pathname !== currentPath) return;
